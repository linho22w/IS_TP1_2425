...
namespace sistema_legado
{
    public partial class Form1 : Form
    {
        ...
        private string apiBaseUrl = "https://localhost:7008/api/Produto"; // Altere para a URL correta
        ...
        //        var produtos = JsonConvert.DeserializeObject<DataTable>(content);

        //        tabelaProdutos.Clear();
        //        tabelaProdutos.Merge(produtos);
        //    }
        //    catch (Exception ex)
        //    {
        //        MessageBox.Show($"Erro ao carregar dados: {ex.Message}");
        ...
        private async void AdicionarProduto(string codigo, DateTime data, TimeSpan hora, string tempo)
        {
            try
            {
                var produto = new
                {
                    ...
                    Hora_Producao = hora,
                    Tempo_Producao = int.Parse(tempo)
                };
            ...
            {
                MessageBox.Show($"Erro na comunicação com a API: {ex.Message}",
        ...
        // Mantenha os seguintes métodos como estão (não precisam de alteração):
        private bool ValidarCampos(string codigo, string tempo)
        {
            string erroMensagem = "";
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!ValidarCampos(codigo, tempo)) return;

                DateTime dataProducao = dtData.Value == DateTime.MinValue ?
                    new DateTime(2000, 1, 1) : dtData.Value;
                TimeSpan horaProducao = dtHora.Value == DateTime.MinValue ?
                    new TimeSpan(0, 0, 0) : dtHora.Value.TimeOfDay;

                AdicionarProduto(codigo, dataProducao, horaProducao, tempo);
                LimparCampos(txtCodigo, txtTempo);
            }
            catch (Exception ex)
            {
